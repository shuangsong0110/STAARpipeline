...
// declare K_Binary_SPA_survival
double K_Binary_SPA_survival(double x, arma::vec muhat, arma::vec G);
// declare K1_Binary_SPA_survival (first derivative)
double K1_Binary_SPA_survival(double x, arma::vec muhat, arma::vec G, double q);
// declare K2_Binary_SPA_survival (second derivative)
double K2_Binary_SPA_survival(double x, arma::vec muhat, arma::vec G);
// declare NR
...
// [[Rcpp::export]]
arma::vec Individual_Score_Test_SPA_survival(arma::mat G, arma::mat XW, arma::mat XXWX_inv, arma::vec residuals, arma::vec muhat, double tol, int max_iter)
{
	int i,k;
	double sum0 = 0.0;
...
double Saddle_Binary_SPA(double q, arma::vec muhat, arma::vec G, double tol, int max_iter, bool lower)
{
    ...
    double w = sqrt(2*(xhat*q - K_Binary_SPA_survival(xhat,muhat,G)));
	
	if((R_finite(w)==0)||(check_is_na(w)))
	{
		w = sqrt(2*(xhat*q - K_Binary_SPA_survival(xhat,muhat,G)));
	}
	...
	ki = xhat*sqrt(K2_Binary_SPA_survival(xhat,muhat,G));
...
double NR_Binary_SPA(arma::vec muhat, arma::vec G, double q, double init, double tol, int max_iter)
{
	...
	if(fabs(K1_Binary_SPA_survival(xi, muhat, G, q)) > tol)
	{
		xi_update = xi - K1_Binary_SPA_survival(xi, muhat, G, q)/K2_Binary_SPA_survival(xi, muhat, G);
	}
	...
	while (!(check_is_na(xi_update))&&(R_finite(xi_update)==1)&&(fabs(xi_update - xi) > tol)&&(fabs(K1_Binary_SPA_survival(xi_update, muhat, G, q)) > tol)&&(no_iter < max_iter))
    {
		no_iter = no_iter + 1;
		xi = xi_update;	
		
		// calculate numerator
		numerator = K1_Binary_SPA_survival(xi, muhat, G, q);
		...
		denominator = K2_Binary_SPA_survival(xi, muhat, G);
...
double Bisection_Binary_SPA(arma::vec muhat, arma::vec G, double q, double xmin, double xmax, double tol)
{
	...
	K1left = K1_Binary_SPA_survival(xlower, muhat, G, q);
	if((R_finite(K1left)==0)||(check_is_na(K1left)))
	{
		K1left = K1_Binary_SPA_survival(xlower, muhat, G, q);
	}
	
	K1right = K1_Binary_SPA_survival(xupper, muhat, G, q);
	...
	{	
		if(!haveSameSign(K1left, K1right))
		{
			while ((fabs(xupper-xlower) > tol)&&(fabs(K1x0) > tol))
			{
				...
				K1x0 = K1_Binary_SPA_survival(x0, muhat, G, q);
...
arma::vec goldenSectionSearchForSignChange(double a, double b, arma::vec muhat, arma::vec G, double q, double tol, int max_iter)
{
	...
	K1x0 = K1_Binary_SPA_survival(x(0), muhat, G, q);
	if((R_finite(K1x0)==0)||(check_is_na(K1x0)))
	{
		K1x0 = K1_Binary_SPA_survival(x(0), muhat, G, q);
	}
	
	K1x1 = K1_Binary_SPA_survival(x(1), muhat, G, q);
...
double K_Binary_SPA_survival(double x, arma::vec muhat, arma::vec G)
{
    ...
    for(int i = 0; i < n; i++)
    {
		...
		res = res + muhat(i) * (exp(x * G(i)) - G(i) *  x -1);
    }
...
double K1_Binary_SPA_survival(double x, arma::vec muhat, arma::vec G, double q)
{
    ...
    for(int i = 0; i < n; i++)
    {
		...
		res = res + muhat(i) * G(i) * (exp(G(i) * x) - 1);
    }
...
double K2_Binary_SPA_survival(double x, arma::vec muhat, arma::vec G)
{
    ...
    for(int i = 0; i < n; i++)
    {
		...
		res = res + muhat(i) * pow(G(i),2.0) * exp(x * G(i));
